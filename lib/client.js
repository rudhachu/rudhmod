...
const { Image, Message, Sticker, Video, AllMessage } = require("./system");
...
const handleMessages = conn => async m => {
	...
	if (config.ANTILINK && msg.from.endsWith("@g.us") && text_msg) {
				...
				if (!isAllowedLink) {
					if (!botIsAdmin) {
						return await conn.sendMessage(msg.from, {
							text: "I need to be an admin to perform antilink actions.",
						});
					}

					const action = antilink.action || "delete";
					await handleAntilinkAction(action, msg, conn);

					if (action === "delete" || action === "all") return;
				}
			}
		  }
	    }
...
async function handleAntilinkAction(action, msg, conn) {
	switch (action) {
		...
		case "warn":
			...
			if (warn.warnCount >= 3) {
				...
				await conn.sendMessage(msg.from, {
					text: `@${msg.sender.split("@")[0]}, you have been warned for sending a link. Warning ${warn.warnCount}/3`,
					mentions: [msg.sender],
				});
			}
			break;
		...
		case "all":
			...
			break;
		default:
			console.log(`Unknown antilink action: ${action}`);
			break;
	}
}
