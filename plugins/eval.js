...
rudhra({on: 'text', fromMe: true, dontAddCommandList: true,desc: 'Run js code (evel)', type: 'misc'}, async (message, match, m, client, msg) => {
...
rudhra({on:'text', fromMe: true,dontAddCommandList: true}, async (message, match, m, client, msg) => {
if (message.text.startsWith("$")) {
var m = message
var conn = message.client
const util = require('util')
const json = (x) => JSON.stringify(x,null,2)
try { let return_val = await eval(`(async () => { ${message.text.replace("$","")} })()`)
